	...
	<body>
		...
		<?php
			...
			if ($result->num_rows > 0) {
				...
				while($row = $result->fetch_assoc()) {
					...
					echo  '<tr><td>'.$row["codi"].'</td><td>'.$row["nom"].'</td><td>'.$row["localitat"].'</td><td>'.$web.'</td><td>'.$escut.'</td><td><a onclick="myFunction('."'".$row["codi"]."'".')"><i class="fas fa-times-circle fa-3x btn" style="color:red"></i></a></td></tr>';
		...
		<script>
			function myFunction(id) {
			  if (confirm("Se borrará el equipo con el codigo: " + id)) {
				 window.location.replace("delete.php?codi="+id);
			   }
			}
		</script>
