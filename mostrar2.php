		...
		<?php
			...
			if ($result->num_rows > 0) {
				...
				while($row = $result->fetch_assoc()) {
					...
					echo  '<text class="h1">'.$row["nom"].'</text>'.$escut;
					echo '<table class="table table-striped table-dark">
						<tr>
							<th>Codi</th>
							<th>Nom de títol</th>
						</tr>';
					$sql2 = "SELECT codi, nom FROM titols WHERE codiequip='".$row['codi']."'";
					$result2 = $conn->query($sql2);
					if ($result2->num_rows > 0) {
						while($row2 = $result2->fetch_assoc()) {
							echo '<tr><td>'.$row2["codi"].'</td><td>'.$row2["nom"].'</td></tr>';
						}
						echo '</table>';
					}
					else {
						echo '<tr><td>#</td><td>No se han encontrado titulos</td></tr>';
						echo '</table>';
					}
				}
			} else {
